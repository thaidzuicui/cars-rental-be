...
const { sendVerificationCode,  verifyCode, clearCode } = require("../middleware/resetPasswordService.js");
const bcrypt = require("bcrypt");
const redisClient = require("../middleware/redisClient");
const {getAsync, delAsync, setexAsync, setAsync} = require("../middleware/redisClient.js");
...
// API sửa profile
router.put("/updateProfile", verifyToken, upload.single("image"), (req, res) => {
  ...
  db.query(checkQuery, [username, email, userId], (err, results) => {
    ...
    db.query(updateQuery, [username, email, full_name, profileImage, userId], (err, result) => {
      if (err) {
        return res.status(500).json({ message: "Database error", error: err });
      }
